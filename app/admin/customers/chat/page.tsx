...
type Session = { id: string; status: Status; source_page: string | null; interested_product: string | null; interested_technology: string | null; visitor_name?: string | null; visitor_phone?: string | null; visitor_avatar?: string | null; last_message_at: string };
...
export default function ChatHistoryPage() {
  ...
  const updateStatus = async (next: Status) => {
    ...
    setSessions((items) => items.map((item) => item.id === selected.session.id ? { ...item, status: next } : item));
  };

  const filtered = useMemo(() => {
    const query = search.trim().toLowerCase();
    if (!query) return sessions;
    return sessions.filter((item) => [item.interested_product, item.interested_technology, item.source_page].some((value) => value?.toLowerCase().includes(query)));
  }, [search, sessions]);

  const statCards = [
    { label: 'Tổng phiên chat', value: sessions.length, icon: MessageSquare, color: 'bg-indigo-50 text-indigo-600' },
    { label: 'Cần xử lý', value: sessions.filter((item) => item.status === 'new').length, icon: Clock3, color: 'bg-orange-50 text-orange-600' },
    { label: 'Đã liên hệ', value: sessions.filter((item) => item.status === 'contacted').length, icon: CheckCircle2, color: 'bg-blue-50 text-blue-600' },
    { label: 'Đã đóng', value: sessions.filter((item) => item.status === 'closed').length, icon: Check, color: 'bg-emerald-50 text-emerald-600' },
  ];
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid h-[620px] min-h-[520px] grid-cols-1 lg:grid-cols-[370px_1fr]">
          <div className="border-b border-slate-100 lg:border-b-0 lg:border-r">
            <div className="flex justify-between border-b border-slate-100 px-5 py-3 text-xs font-black uppercase tracking-wide text-slate-400"><span>Danh sách phiên</span><span>{filtered.length}</span></div>
            {loading ? <div className="p-10 text-center text-sm text-slate-400">Đang tải lịch sử...</div> : filtered.length === 0 ? <div className="p-10 text-center"><MessageSquare size={42} className="mx-auto mb-3 text-slate-200" /><p className="text-sm text-slate-500">Chưa có lịch sử chat.</p></div> : filtered.map((item) => <button key={item.id} onClick={() => openSession(item.id)} className={`w-full border-b border-slate-100 p-5 text-left transition hover:bg-slate-50 ${selected?.session.id === item.id ? 'border-l-4 border-l-indigo-500 bg-indigo-50/50 pl-4' : ''}`}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-indigo-100 text-indigo-600">{item.visitor_avatar ? <Image src={item.visitor_avatar} alt="" fill sizes="36px" className="object-cover" /> : <span className="font-bold">{(item.visitor_name || 'K').slice(0, 1).toUpperCase()}</span>}</span><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-800">{item.visitor_name || 'Khách truy cập ẩn danh'}</p><p className="mt-1 truncate text-xs text-slate-400">{item.interested_product || item.interested_technology || 'Chưa xác định nhu cầu'}</p></div></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${statusClass[item.status]}`}>{labels[item.status]}</span></div><div className="mt-3 flex justify-between gap-2 text-[11px] text-slate-400"><span className="truncate">{item.interested_technology || item.source_page || 'Không rõ trang'}</span><span className="shrink-0">{time(item.last_message_at)}</span></div></button>)}
          </div>
